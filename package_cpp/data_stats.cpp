...
void poly_fit(const int n,const string output_file, VECTOR &xx,VECTOR &yy,VECTOR & ww){
    ...
    FILE* out = fopen(&output_file[0],"w");
    fprintf(out,"Pearson's r = %lg\n",r);
    fprintf(out,"ν = %d\n",N-n-1);
    fprintf(out,"𝜒2/ν = %lg\n",chi2/(N-n-1));
    fprintf(out,"Coefficients:\n");
    for(int i=0;i<n+1;i++)
        fprintf(out,"a%d = %lg +/- %lg\n",i,coeff[i],coeff_err[i]);

    // Covariance matrix
    fprintf(out,"Covariance matrix:\n");
    for(int i=0;i<n+1;i++){
        for(int j=0;j<n+1;j++){
