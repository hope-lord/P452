...
int main(void){
	...
	while(!converged_A or !converged_B ){
		...
		for_each(execution::par,index.begin(),index.end(),[&](auto i){
    ...
    fprintf(data_file,"# x A(x) B(x) M(x)\n");
    for(int i=0;i<N;i++)
        fprintf(data_file,"%lg %lg %lg %lg\n",tx[i],A[i],B[i],B[i]/A[i]);
    fclose(data_file);
